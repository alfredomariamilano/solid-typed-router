...
const typedSearchParamsTemplatePath = path.resolve(dirname, "..", "static", "typedSearchParams.template.ts");
...
const generateTypedRoutes = async (resolvedOptions_) => {
	...
	try {
		...
		if (!fs.existsSync(routesPath) || !fs.lstatSync(routesPath).isDirectory()) throwError(`Routes directory not found at ${routesPath}`);
		...
		if (routesDefinitions.length <= 0) try {
			...
			const routesFilesPaths = fs.readdirSync(routesPath, {
				recursive: true,
				encoding: "utf-8"
			...
			for (let i = 0; i < output.length; i++) {
				...
				if (file.facadeModuleId) {
					...
					if (isRoute) {
						...
						let relativePathFromTypedRouter = path.relative(path.dirname(resolvedOptions.typedRouterPath), path.join(resolvedOptions.routesPath, relativePath)).replace(/\\/g, "/");
						...
						if (isValidRoute) {
							...
							if (routePath) {
								...
								if (routePath === "/" || !routePath.endsWith("/")) set(routesObject, routePartsReplacedWithRoute, routePath);
							}
							...
							if (file.exports.includes("searchParams") && !resolvedOptions.searchParamsSchemas[routePath]) {
								...
								let relativePathFromTypedSearchParams = path.relative(path.dirname(resolvedOptions.typedSearchParamsPath), path.join(resolvedOptions.routesPath, relativePath)).replace(/\\/g, "/");
